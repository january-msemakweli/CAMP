"use strict";Object.defineProperty(exports, "__esModule", {value: true}); function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; } function _nullishCoalesce(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } } async function _asyncNullishCoalesce(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return await rhsFn(); } } function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }var b="0.3.5";var _mcputils = require('@supabase/mcp-utils');var _zod = require('zod');var _commontags = require('common-tags');function A(s,r=100){switch(s){case"api":return _commontags.stripIndent`
      ...
      `;case"realtime":return _commontags.stripIndent`
        ...
        order by timestamp desc
        limit ${r}
      `;default:throw new Error(`unsupported log service type: ${s}`)}}var _openapifetch = require('openapi-fetch'); var _openapifetch2 = _interopRequireDefault(_openapifetch);function j(s,r,n={}){return _openapifetch2.default.call(void 0, {baseUrl:s,headers:{"Content-Type":"application/json",Authorization:`Bearer ${r}`,...n}})}var F=_zod.z.object({message:_zod.z.string()});function l(s,r){if("error"in s){if(s.response.status===401)throw new Error("Unauthorized. Please provide a valid access token to the MCP server via the --access-token flag.");let{data:n}=F.safeParse(s.error);throw n?new Error(n.message):new Error(r)}}var G="ABCDEFGHIJKLMNOPQRSTUVWXYZ",W="abcdefghijklmnopqrstuvwxyz",q="0123456789",B="!@#$%^&*()_+~`|}{[]:;?><,./-=",w=({length:s=10,numbers:r=!1,symbols:n=!1,uppercase:g=!0,lowercase:d=!0}={})=>{let p="";if(g&&(p+=G),d&&(p+=W),r&&(p+=q),n&&(p+=B),p.length===0)throw new Error("at least one character set must be selected");let e=new Uint32Array(s);crypto.getRandomValues(e);let a="";for(let o=0;o<s;o++){let c=e[o]%p.length;a+=p.charAt(c)}return a};var C=`-- Adapted from information_schema.columns
...
group by
  ...
  nc.nspname,
  pk.primary_keys
`;var Y=["information_schema","pg_catalog","pg_toast"];function L(s=[]){let r=_commontags.stripIndent`
    with
      tables as (${R}),
      columns as (${C})
    select
    ...
    from tables
  `;return s.length>0?r+=`  where schema in (${s.map(n=>`'${n}'`).join(",")})`:r+=`  where schema not in (${Y.map(n=>`'${n}'`).join(",")})`,r}function I(){return O}var J=(s,r)=>_commontags.stripIndent`
    COALESCE(
      (
        SELECT
